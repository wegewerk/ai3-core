        ...
        return new AjaxRequest(TYPO3.settings.ajaxUrls[endpoint])
            ...
            .then(async function (response) {
                ...
                if (returnJson) {
                    ...
                    const responseBody = JSON.parse(resolved);
                    if (responseBody.error) {
                        Notification.error(TYPO3.lang['ai3.notification.generation.requestError'], responseBody.error);
                        return null;
                    } else {
                        return responseBody;
                    }
            ...
            .catch((error) => {
                Notification.error(TYPO3.lang['ai3.notification.generation.error'], error.statusText);
                return null;
            });
    }
}
